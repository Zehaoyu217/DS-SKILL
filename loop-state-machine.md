...
| VALIDATE | no leakage patterns active; CV results with uncertainty present; nested-CV for any tuned hyperparameters; `plans/v{N}-updates.md` has `status: closed` with summary block | #3 (CV scheme), #9 (encoding), #10 (plan log closed) | Same |
